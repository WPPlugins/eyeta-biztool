...
function ajax_get_listviewcell_base() {

	global $eyeta_biztool;
	eyeta_biztool_log('ajax_get_listviewcell_base start');
	$nonce = $_REQUEST['_wpnonce'];
	if(! $eyeta_biztool->verify_nonce( $nonce ) ) {
		eyeta_biztool_log('ajax_get_listviewcell_base nonce error');
	...
	if($eyeta_biztool->current_user_can('eyeta-biztool/setting/table')) {
		if(isset($_REQUEST['num']) && '' != $_REQUEST['num'] && is_numeric($_REQUEST['num']) && 0 < intval($_REQUEST['num'])) {
			...
			setting_table_listviewcell_base($_REQUEST['post_type'], $_REQUEST['num'], $_REQUEST['listview_num']);
		} else {
