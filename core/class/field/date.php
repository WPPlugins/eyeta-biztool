 ...
 */
class Biztool_Field_Date extends Biztool_Field {
	...
	function get_search_str($target_post_id) {
		...
		if($str_field) {
			$rsl_field = substr($str_field, 0, 4) . '/' . substr($str_field, 4,2) . '/' . substr($str_field, 6, 2);
		} else {
			$rsl_field='';
		}
		return ($str_field);
	...
	function get_inlineedit_column($target_post_id) {
		...
		$str_html .= '<input type="text" org="' . esc_attr($str_field) 
			...
			. $target_post_id . '" class="diff-check datepicker no-form w100" value="'
	...
	function wp_add_inline_script_vars_for_list($post_type, $target_post_id) {
		...
		$js .= '$("#' . $this->_vars['field_key'] . '-' . $target_post_id . '").mask("' . apply_filters('eyeta_biztool/field/calendar/mask', '9999/99/99', $this) . '");';
		$js .= '});</script>';
		echo $js;
	...
	function wp_add_inline_script_vars($post_type, $array_js_vars = array()) {
		...
		// マスクインプットがあればセット　'eyeta-biztool-maskinput'
		$js = 'jQuery(function($){';
		$js .= '$("#' . $this->_vars['field_key'] . '").mask("' . apply_filters('eyeta_biztool/field/calendar/mask', '9999/99/99', $this) . '");';
		$js .= '});';
	...
	function get_field_edit_form($mb, $html = '') {
		...
		<input type="text" name="<?php $mb->the_name(); ?>" class="datepicker" id="<?php echo esc_attr($this->_vars['field_key']); ?>" placeholder="<?php echo esc_attr($this->_vars['additional_settings']['placeholder']);?>" value="<?php if($mb->have_value()) { $mb->the_value(); } ?>" />
