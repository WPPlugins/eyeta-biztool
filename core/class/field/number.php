 ...
 */
class Biztool_Field_Number extends Biztool_Field {
	...
	function __construct($post_type = null, $field_num = null) {
		...
		$this->_vars['additional'] = array(
			...
			'max' => array( 'type' => 'number', 'default' => '', 'required' => false),
			'min' => array( 'type' => 'number', 'default' => '', 'required' => false)
		);
	...
	function get_simple_column($target_post_id) {
		...
		if($str_field) {
			if(strpos($str_field, '.') !== false) {
				// 小数有り
				$array_tmp = explode('.', $str_field);
				$str = number_format($array_tmp[0]) . '.' . $array_tmp[1];
				return $str;
			} else {
				$str = number_format($str_field);
				return $str;
			}
		}
	}
	...
	function get_inlineedit_column($target_post_id) {
		...
		$str_field = get_post_meta($target_post_id, $this->_vars['field_key'], true);
		$str_html .= '<input type="number" org="' . esc_attr($str_field)
	...
	function wp_add_inline_script_vars_for_list($post_type, $target_post_id) {
		...
		$array_rule['number']=true;
		if($this->_vars['additional_settings']['max']) {
			$array_rule['max'] = 0.0 + $this->_vars['additional_settings']['max'];
		}
		if($this->_vars['additional_settings']['min']) {
			$array_rule['min'] = 0.0 + $this->_vars['additional_settings']['min'];
		}
		if($array_rule) {
	...
	function get_field_edit_form($mb, $html = '') {
		...
		<input type="number" name="<?php $mb->the_name(); ?>" id="<?php echo esc_attr($this->_vars['field_key']); ?>" placeholder="<?php echo esc_attr($this->_vars['additional_settings']['placeholder']);?>" value="<?php if($mb->have_value()) { $mb->the_value(); } else { echo esc_attr($this->_vars['additional_settings']['default_value']); } ?>" />
	...
	function get_field_form($html = '') {
		...
		<tr class="field-form-detail">
			<td class="fbold"><?php echo esc_html__('Maximum value', 'eyeta-biztool');?><br /><span class="description"><?php echo esc_html__('If you do not want to restrict in the blank', 'eyeta-biztool');?></span></td>
			<td class=""><input type="number" class="w100" name="max-<?php echo $this->field_num;?>" value="<?php echo esc_attr($this->get_additional_values_for_form('max'));?>" /></td>
		</tr>
		<tr class="field-form-detail">
			<td class="fbold"><?php echo esc_html__('Minimum value', 'eyeta-biztool');?><br /><span class="description"><?php echo esc_html__('If you do not want to restrict in the blank', 'eyeta-biztool');?></span></td>
			<td class=""><input type="number" class="w100" name="min-<?php echo $this->field_num;?>" value="<?php echo esc_attr($this->get_additional_values_for_form('min'));?>" /></td>
		</tr>
