 ...
 */
class Biztool_Field_Textarea extends Biztool_Field {
	...
	function __construct($post_type = null, $field_num = null) {
		...
		$this->_vars['field_type'] = 'textarea';
	...
	function get_inlineedit_column($target_post_id) {
		...
		$str_html .= '<textarea org="' . esc_attr($str_field)
			. '" name="' . esc_attr($this->_vars['field_key']) . '-'
			. $target_post_id . '" id="' . esc_attr($this->_vars['field_key']) . '-'
			. $target_post_id . '" class="diff-check no-form w100"  target-post_id="'
			. $target_post_id . '" target-field-key="' . esc_attr($this->_vars['field_key']) . '" >' . esc_attr($str_field) . '</textarea>';
	...
	function get_field_edit_form($mb, $html = '') {
		...
		<textarea name="<?php $mb->the_name(); ?>" id="<?php echo esc_attr($this->_vars['field_key']); ?>" placeholder="<?php echo esc_attr($this->_vars['additional_settings']['placeholder']);?>" ><?php if($mb->have_value()) { $mb->the_value(); } else { echo esc_html($this->_vars['additional_settings']['default_value']); } ?></textarea>
	...
	function get_simple_column($target_post_id) {
		...
		return nl2br(esc_html($str_field));
	}
